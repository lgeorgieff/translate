...
TEST(command_line_parser, language_arguments) {
  CommandLineParser clp = CommandLineParser{2, (const char* []){"app", "--all-languages"}};
  EXPECT_TRUE(clp.all_languages());
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--language-id", "PL"}};
  EXPECT_TRUE(clp.has_language_id());
  EXPECT_EQ(clp.language_id(), string{"PL"});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--language-name", "polski"}};
  EXPECT_TRUE(clp.has_language_name());
  EXPECT_EQ(clp.language_name(), string{"polski"});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--all-languages", "only a phrase"}};
  EXPECT_TRUE(clp.all_languages());
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--language-id", "PL", "only a phrase"}};
  EXPECT_TRUE(clp.has_language_id());
  EXPECT_EQ(clp.language_id(), string{"PL"});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--language-name", "polski", "only a phrase"}};
  EXPECT_TRUE(clp.has_language_name());
  EXPECT_EQ(clp.language_name(), string{"polski"});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--language-id"}}), CommandLineException);
  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--language-name"}}), CommandLineException);
}

TEST(command_line_parser, word_class_arguments) {
  CommandLineParser clp = CommandLineParser{2, (const char* []){"app", "--all-word-classes"}};
  EXPECT_TRUE(clp.all_word_classes());
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--word-class-id", "noun"}};
  EXPECT_TRUE(clp.has_word_class_id());
  EXPECT_EQ(clp.word_class_id(), string{"noun"});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--word-class-name", "noun"}};
  EXPECT_TRUE(clp.has_word_class_name());
  EXPECT_EQ(clp.word_class_name(), string{"noun"});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--all-word-classes", "only a phrase"}};
  EXPECT_TRUE(clp.all_word_classes());
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--word-class-id", "noun", "only a phrase"}};
  EXPECT_TRUE(clp.has_word_class_id());
  EXPECT_EQ(clp.word_class_id(), string{"noun"});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--word-class-name", "noun", "only a phrase"}};
  EXPECT_TRUE(clp.has_word_class_name());
  EXPECT_EQ(clp.word_class_name(), string{"noun"});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--word-class-id"}}), CommandLineException);
  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--word-class-name"}}), CommandLineException);
}

TEST(command_line_parser, gender_arguments) {
  CommandLineParser clp = CommandLineParser{2, (const char* []){"app", "--all-genders"}};
  EXPECT_TRUE(clp.all_genders());
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--gender-id", "f."}};
  EXPECT_TRUE(clp.has_gender_id());
  EXPECT_EQ(clp.gender_id(), string{"f."});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--gender-name", "female"}};
  EXPECT_TRUE(clp.has_gender_name());
  EXPECT_EQ(clp.gender_name(), string{"female"});
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--all-genders", "only a phrase"}};
  EXPECT_TRUE(clp.all_genders());
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--gender-id", "m.", "only a phrase"}};
  EXPECT_TRUE(clp.has_gender_id());
  EXPECT_EQ(clp.gender_id(), string{"m."});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  clp = CommandLineParser{4, (const char* []){"app", "--gender-name", "male", "only a phrase"}};
  EXPECT_TRUE(clp.has_gender_name());
  EXPECT_EQ(clp.gender_name(), string{"male"});
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});

  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--gender-id"}}), CommandLineException);
  EXPECT_THROW((clp = CommandLineParser{2, (const char* []){"app", "--gender-name"}}), CommandLineException);
}

TEST(command_line_parser, numeri_arguments) {
  CommandLineParser clp = CommandLineParser{2, (const char* []){"app", "--all-numeri"}};
  EXPECT_TRUE(clp.all_numeri());
  EXPECT_EQ(clp.phrase(), string{""});

  clp = CommandLineParser{3, (const char* []){"app", "--all-numeri", "only a phrase"}};
  EXPECT_TRUE(clp.all_numeri());
  EXPECT_EQ(clp.phrase(), string{"only a phrase"});
}

TEST(command_line_parser, usage) {
  // TODO: implement
}
